...
namespace KCS
{
    ...
    class StartUpProcess : IProcess, IDisposable
    {
        ...
        {
            if (startCore != null)
                startCore.Invoke(this, new ProcessStatusEventArgs(status));
        }
        void RaiseRunning(string status)
        {
            if (runningCore != null)
                runningCore.Invoke(this, new ProcessStatusEventArgs(status));
        }
        void RaiseComplete()
        {
            if (completeCore != null)
                completeCore.Invoke(this, EventArgs.Empty);
        }
        #endregion
    }
}
