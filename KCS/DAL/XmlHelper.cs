...
namespace KCS.Common.DAL
{
    class XmlHelper
    {
        ...
        public static string GetWorkDirectory()
        {
            try
            {
                return Path.GetDirectoryName(typeof(XmlHelper).Assembly.Location);
            }
            catch
            {
        ...
        public static int GetConfigData(string szKeyName, int nDefaultValue, string xmlFileName)
        {
            string szValue = GetConfigData(szKeyName, nDefaultValue.ToString(), xmlFileName);
            try
            {
        ...
        /// <param name="szDefaultValue">指定的Key不存在时,返回的值</param>
        /// <returns>Key值</returns>
        public static float GetConfigData(string szKeyName, float fDefaultValue, string xmlFileName)
        {
            string szValue = GetConfigData(szKeyName, fDefaultValue.ToString(), xmlFileName);
            try
            {
        ...
        /// <param name="szDefaultValue">指定的Key不存在时,返回的值</param>
        /// <returns>Key值</returns>
        public static bool GetConfigData(string szKeyName, bool bDefaultValue, string xmlFileName)
        {
            string szValue = GetConfigData(szKeyName, bDefaultValue.ToString(), xmlFileName);
            try
            {
        ...
        /// <param name="szDefaultValue">指定的Key不存在时,返回的值</param>
        /// <returns>Key值</returns>
        public static string GetConfigData(string szKeyName, string szDefaultValue, string xmlFileName)
        {
            string szConfigFile = string.Format("{0}\\{1}", GetWorkDirectory(), xmlFileName);
        ...
        public static bool WriteConfigData(string szKeyName, string szValue, string xmlFileName)
        {
            string szConfigFile = string.Format("{0}\\{1}", GetWorkDirectory(), xmlFileName);
            if (!File.Exists(szConfigFile))
            {
                if (!CreateXmlFile(szConfigFile, "KCS_Config"))
                    return false;
            }
            XmlDocument clsXmlDoc = GetXmlDocument(szConfigFile);
